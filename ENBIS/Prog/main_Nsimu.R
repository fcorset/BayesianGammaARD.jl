...
# Nb de simulations
N<- 1
est.theta <- matrix(nrow=N,ncol=6)
...
L <- 5 # seuil pour MP
M <- 10 # seuil pout MC
tps.final <- 50 # fenêtre d'observation du processus
...
for(kk in 1:N){
  ...
  while (j<=nb.inspections) {
    ...
    y[((j-1)*tau/pas+2):(j*(tau/pas)+1)]<-y[(j-1)*tau/pas+1]+ x[nb.cycles,((j-1)*tau/pas+2):(j*(tau/pas)+1)]- x[nb.cycles,(j-1)*tau/pas+1]
    ...
    if (obs[j] < L) {
      ...
      if (obs[j]<M) {
        u<-runif(1)
        if (u>p){
          vect.b[j]<-0 # b=0 si worse, =1 si efficace
        }
  ...
  plot(temps,y,type="l",ylim=c(0,max(y.tilde)),xlim = c(0,tps.final),ylab="",xlab="")
  ...
  hat.theta.new<-matrix(nrow=K+1,ncol=6)
  
  #hat.theta[1,] <- c(1.2,1.1,1.5,0.8,0.9,0.6) # (alpha,beta,b,rho,rho^\prime,p)
  hat.theta[1,] <- c(1,1,1,.9,.2,0.8)
  hat.theta.new[1,] <- c(1,1,1,.9,.2,0.8)
  ...
  delta.new <- hat.theta.new[1,1]*(tps.insp.wR^hat.theta.new[1,2]-tps.insp.pre.wR^hat.theta.new[1,2])
  ...
  p.tilde.new <- ifelse(data1$obs/data1$obs.pre<=1,1,(hat.theta.new[1,6]*dgamma(data1$obs-(1-hat.theta.new[1,4])*data1$obs.pre,delta.new[ind.i.u==1],rate=hat.theta.new[1,3]))/(hat.theta.new[1,6]*dgamma(data$obs[ind.i.u==1]-(1-hat.theta.new[1,4])*data$obs.pre[ind.i.u==1],delta.new[ind.i.u==1],rate=hat.theta.new[1,3])+(1-hat.theta.new[1,6])*dgamma(data$obs[ind.i.u==1]-(1+hat.theta.new[1,5])*data$obs.pre[ind.i.u==1],delta.new[ind.i.u==1],rate=hat.theta.new[1,3])))
  
  
  
  data.temp<-data.frame(data1,p.tilde)
  data1.rhow<-filter(data.temp,p.tilde<1)
  data0.rhow<-filter(data.temp,p.tilde==1)
  
  # Ajout du 16/11/22 : filtrer lorsque obs < obs.pre pour estimer rho !
  
  data1.rho <- filter(data1,data1$obs<data1$obs.pre)
  ...
  for (k in 2:(K+1)){
    ...
    hat.theta.new[k,6]<-mean(p.tilde.new) # mise à jour de p
    ...
    # theta <-c(alpha,beta,b,rho,rho_w,p)
    hat.theta[k,4:5]<-c(lower.rho,upper.rhow)
    
    # 10/11/2022 : On peut estimer rho pour toutes les obs tq obs<obs.pre
    
    esp.logL.abc <- function(x){
      # x[1] : alpha
      # x[2] : beta
      # x[3] : b
      # Cette function calcule la log vraisemblance tq obs < obs.pre
      sum(log(dgamma(data0$obs-data0$obs.pre,shape =x[1]*(data0$tps.insp.wR^x[2]-data0$tps.insp.pre.wR^x[2]),rate = x[3])))
    }
    par <- hat.theta[k-1,1:3]
    hat.theta.new[k,1:3] <- optim(par,esp.logL.abc,control = list(fnscale=-1))$par
    # Mise à jour des delta
    delta.new <- hat.theta.new[k,1]*(tps.insp.wR^hat.theta.new[k,2]-tps.insp.pre.wR^hat.theta.new[k,2])
...
#      part.1 <- sum(p.tilde*log(dgamma(x = data1$obs-(1-hat.theta[k,4])*data1$obs.pre,shape=x[1]*(data1$tps.insp.wR^x[2]-data1$tps.insp.pre.wR^x[2]),rate=x[3])))
      #part.1.w <- sum((1-data1.rhow$p.tilde)*log(dgamma(x = data1.rhow$obs-(1+x[5])*data1.rhow$obs.pre,shape=x[1]*(data1.rhow$tps.insp.wR^x[2]-data1.rhow$tps.insp.pre.wR^x[2]),rate=x[3])))
#      part.1.w <- sum((1-data1.rhow$p.tilde)*log(dgamma(x = data1.rhow$obs-(1+hat.theta[k,5])*data1.rhow$obs.pre,shape=x[1]*(data1.rhow$tps.insp.wR^x[2]-data1.rhow$tps.insp.pre.wR^x[2]),rate=x[3])))
#      return(part.0+part.1+part.1.w)
      return(part.0)
    }
    #par <- hat.theta[k-1,1:5]
    par <- hat.theta[k-1,1:3]
    #hat.theta[k,1:5] <- optim(par,esp.logL,method = "L-BFGS-B",lower=c(10^(-1),10^(-1),10^(-1),10^(-1),10^(-1)),upper=c(10,10,10,10,10),control = list(fnscale=-1))$par
    #hat.theta[k,1:5] <- optim(par,esp.logL,control = list(fnscale=-1))$par
    hat.theta[k,1:3] <- optim(par,esp.logL,control = list(fnscale=-1))$par
    ...
    print(paste("la valeur de alpha à l'étape ",k," est : ",round(hat.theta[k,1],2)," (vraie valeur : ",theta[1],")"))
    print(paste("la valeur de beta à l'étape ",k," est : ",round(hat.theta[k,2],2)," (vraie valeur : ",theta[2],")"))
    print(paste("la valeur de b à l'étape ",k," est : ",round(hat.theta[k,3],2)," (vraie valeur : ",theta[3],")"))
    print(paste("la valeur de rho à l'étape ",k," est : ",round(hat.theta[k,4],2)," (vraie valeur : ",theta[4],")"))
    print(paste("la valeur de rho_w à l'étape ",k," est : ",round(hat.theta[k,5],2)," (vraie valeur : ",theta[5],")"))
    print(paste("la valeur de p à l'étape ",k," est : ",round(hat.theta[k,6],2)," (vraie valeur : ",theta[6],")"))
    print("==============================================================")
  }
  est.theta[kk,] <-hat.theta[K+1,]
