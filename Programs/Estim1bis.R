estim1 <- function(D, tau, L, M, tps.final, pas, optim.method = "SANN", guess = FALSE, eps = 0.01) {
    ...
    for (i in 1:nb.cycles) {
      ...
      for (j in 2:nb) {
        if (!Obs[[i]]$action[j-1]) {
          ...
          # if (accr[j-1] <0) {
          #   cat("j = ", j, "\n")
          #   cat("Action" : Obs[[i]]$action[j-1], "\n")
          #   cat("Obs avant : ", Obs[[i]]$obs[j-1], "\n")
          #   cat("Obs après : ", Obs[[i]]$obs[j], "\n")
          # }
        } else {
          accr[j-1] <- Obs[[i]]$obs[j] - (1-rho)*Obs[[i]]$obs[j-1]
          # if (accr[j-1] < 0) {
