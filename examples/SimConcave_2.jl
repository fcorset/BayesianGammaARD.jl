...
df10_rho0_2 = CSV.read("examples/Results/Concave/sim_data_full_$rho_label.csv", DataFrame)[1:10,:]
df25_rho0_2 = CSV.read("examples/Results/Concave/sim_data_full_$rho_label.csv", DataFrame)[1:25,:]
df50_rho0_2 = CSV.read("examples/Results/Concave/sim_data_full_$rho_label.csv", DataFrame)[1:50,:]
df75_rho0_2 = CSV.read("examples/Results/Concave/sim_data_full_$rho_label.csv", DataFrame)[1:75,:]
df100_rho0_2 = CSV.read("examples/Results/Concave/sim_data_full_$rho_label.csv", DataFrame)[1:100,:]
df125_rho0_2 = CSV.read("examples/Results/Concave/sim_data_full_$rho_label.csv", DataFrame)[1:125,:]
df150_rho0_2 = CSV.read("examples/Results/Concave/sim_data_full_$rho_label.csv", DataFrame)[1:150,:]
df175_rho0_2 = CSV.read("examples/Results/Concave/sim_data_full_$rho_label.csv", DataFrame)[1:175,:]
df200_rho0_2 = CSV.read("examples/Results/Concave/sim_data_full_$rho_label.csv", DataFrame)[1:200,:]
...
for i in 1:length(ρlow)
    ...
    println("Lower bound for ρ (n=$(nrow(dfs[i]))): $(ρlow[i])")
    ...
    d = c * (1/priormeanβ - 1)
    println("Parameters for β: c = $c, d = $d")

    dβ = Informative(:β, c, d, 1) # le troisième paramètre est égal à 1 pour loi beta
    ...
    if g > 0 && h > 0
        ...
        priorvarρ = 0.02  # Variance réduite
    ...
    if g_bad > 0 && h_bad > 0
        ...
        println("Warning: Invalid parameters for Beta distribution.")
        badpriorvarρ = 0.02  # Variance réduite
        g_bad = (ρlow[i]-badpriormeanρ)*(ρlow[i]-ρlow[i]*badpriormeanρ-badpriormeanρ+badpriormeanρ^2+badpriorvarρ)/(badpriorvarρ*(1-ρlow[i]))
        h_bad=-(1-badpriormeanρ)*(ρlow[i]-ρlow[i]*badpriormeanρ-badpriormeanρ+badpriormeanρ^2+badpriorvarρ)/(badpriorvarρ*(1-ρlow[i]))    
    end
...
CSV.write("examples/Results/Concave/sim_data_full_$rho_label.csv", df200_rho0_2)

n_values = [10,25,50,75,100,125,150,175,200]
for i in 1:length(res_NI_all_rho0_2)
    CSV.write("examples/Results/Concave/bayes_posterior_non_informative_n$(n_values[i])_$rho_label.csv", res_NI_all_rho0_2[i])
    CSV.write("examples/Results/Concave/bayes_posterior_good_priors_n$(n_values[i])_$rho_label.csv", res_GP_all_rho0_2[i])
    CSV.write("examples/Results/Concave/bayes_posterior_bad_priors_n$(n_values[i])_$rho_label.csv", res_BP_all_rho0_2[i])
end
...
CSV.write("examples/Results/Concave/mle_results_$rho_label.csv", mle_df_rho_0_2)
