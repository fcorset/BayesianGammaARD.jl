...
for i in 1:length(ρlow)
    ...
    println("Lower bound for rho with i=$i and rho0=0.7: ", ρlow[i])
...
CSV.write("examples/Results/Convex/sim_data_full_$rho_label.csv", df200_rho0_7)

n_values = [10,25,50,75,100,125,150,175,200]
for i in 1:length(res_NI_all_rho0_7)
    CSV.write("examples/Results/Convex/bayes_posterior_non_informative_n$(n_values[i])_$rho_label.csv", res_NI_all_rho0_7[i])
    CSV.write("examples/Results/Convex/bayes_posterior_good_priors_n$(n_values[i])_$rho_label.csv", res_GP_all_rho0_7[i])
end
